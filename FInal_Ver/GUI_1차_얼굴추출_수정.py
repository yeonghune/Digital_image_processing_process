...
class WindowClass(QMainWindow, form_class):
    ...
    def Make_Dataset(self):
        ...
        if not self.FileExist:
            print("\n[얼굴 학습] 등록된 얼굴 사진이 없기 때문에 학습을 할 수 없습니다.")
            self.textBrowser.append("\n[얼굴 학습] 등록된 얼굴 사진이 없기 때문에 학습을 할 수 없습니다.")
        else:
    ...
    def run(self):
        ...
        if self.ThRun:
            if howmanyFiles == 0:
                ...
                print("got faces and ids")
                self.recognizer.train(faces, np.array(ids))
                self.recognizer.write('trainer/haar_trainer_GUI.yml')
